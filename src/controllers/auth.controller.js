...
const register = catchAsync(async (req, res) => {
    ...
    if(!Object.keys(result).length && !Object.keys(tokens).length)
    {
      throw new ApiError(httpStatus[400],'unable to process request');
    }
    res.status(httpStatus.CREATED).send({"user":result,tokens})
...
const login = catchAsync(async (req, res) => {
  ...
  if(!Object.keys(user).length && !Object.keys(tokens).length)
    {
      throw new ApiError(httpStatus[400],'unable to process request');
    }
